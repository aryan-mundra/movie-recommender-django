...
def detail(request, movie_id):
    ...
    if request.method == "POST":
        ...
        # For rating
            ...
            rate = request.POST.get('rating', None)  # Use get with a default value
            if rate is not None:
                ...
                messages.error(request, "Rating value not found in the request.")
...
def recommend(request):
    ...
    similar_movies = pd.concat([get_similar(movie, rating, corrMatrix) for movie, rating in user_filtered])

    # Convert the NumPy Series to a Pandas Series before sorting
    similar_movies = pd.Series(similar_movies)
